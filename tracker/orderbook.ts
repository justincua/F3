...
export class OrderBook {
  ...
  /** Resolves when both UP and DOWN books have received their initial snapshot.
   *  V13: never hang forever on Railway WS hiccups; return false after timeout
   *  so the strategy can keep diagnostics live and continue waiting/reconnecting. */
  waitForReady(timeoutMs = Number(process.env.ORDERBOOK_READY_TIMEOUT_MS ?? 3500)): Promise<boolean> {
    return new Promise<boolean>((resolve) => {
      ...
      const pollMs = Math.max(15, Number(process.env.ORDERBOOK_READY_POLL_MS ?? ((process.env.ULTRA_FAST_MODE ?? "").toLowerCase() === "true" ? 25 : 100)));
  ...
  isReady(): boolean {
    return this.books.has(this.assetIds[0]!) && this.books.has(this.assetIds[1]!);
  }
