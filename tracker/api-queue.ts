  ...
  queueMarketPrice(slot: Slot): { cancel: () => void } {
    ...
    fetchWithRetry(url, {
      ...
      retryBackOff: (currentRetry) => {
        ...
        const fastRetry = Math.max(50, Number(process.env.MARKET_PRICE_RETRY_MS ?? (fastMode ? 120 : 500)));
        const afterOpenRetry = Math.max(1000, Number(process.env.MARKET_PRICE_AFTER_OPEN_RETRY_MS ?? (fastMode ? 2500 : 8000)));
        // V14: around slot open, poll the price API much faster so Price-to-beat appears quickly.
        if (this.marketResult.get(slot.startTime)) {
          return afterOpenRetry;
        }
        return fastMode ? fastRetry : currentRetry * fastRetry;
      },
    });
