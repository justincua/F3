...
const AUTO_RESTART_ON_CLEAN_EXIT = boolEnv(process.env.AUTO_RESTART_ON_CLEAN_EXIT, AUTO_START_BOT);
const BOT_WATCHDOG_MS = Math.max(5000, Number(process.env.BOT_WATCHDOG_MS ?? 15000));
...
function startBot(params: Record<string, any>, source: "manual" | "auto" = "manual") {
  ...
  botProcess.exited.then((code: number) => {
    ...
    const control = readBotControl();
    const canRestartCrash =
      AUTO_RESTART_BOT &&
      code !== 0 &&
      !control.paused &&
      !control.stopRequested &&
      autoRestartCount < AUTO_RESTART_MAX;
    const canRestartClean =
      AUTO_RESTART_ON_CLEAN_EXIT &&
      code === 0 &&
      !control.paused &&
      !control.stopRequested;
    ...
    } else if (canRestartClean) {
      autoRestartCount = 0;
      lastControllerMessage = `Bot exited code 0 nhưng AUTO_START vẫn bật. V13 tự chạy lại để không bỏ phiên.`;
      scheduleAutoStart(lastControllerMessage);
    } else if (code === 0) {
      autoRestartCount = 0;
    }
...
// V13 safety net: if Railway/server is alive but child bot is gone, restart it.
setInterval(() => {
  if (!AUTO_START_BOT || isBotProcessRunning() || autoRestartTimer) return;
  const control = readBotControl();
  if (control.paused || control.stopRequested) return;
  lastControllerMessage = "V13 watchdog: bot process không chạy, auto-start lại để không bỏ phiên.";
  console.log(`[watchdog] ${lastControllerMessage}`);
  scheduleAutoStart("watchdog", 1000);
}, BOT_WATCHDOG_MS);
...
const server = Bun.serve({
  ...
  async fetch(req: Request) {
    ...
    if (url.pathname === "/api/state") {
      return Response.json(loadState(), {
        headers: {
          ...
          "pragma": "no-cache",
          "expires": "0",
          "x-cuax-realtime": process.env.REALTIME_EVERYTHING === "true" ? "true" : "false",
        },
      });
    }
    ...
    if (url.pathname === "/") {
      return new Response(loadHtml(), { headers: { "content-type": "text/html; charset=utf-8", "cache-control": "no-store, no-cache, must-revalidate", "pragma": "no-cache", "expires": "0" } });
    }
