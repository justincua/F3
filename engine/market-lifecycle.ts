...
export class MarketLifecycle {
  ...
  private _lastSetupAttemptMs = 0;
  private _lastSetupLogMs = 0;
  ...
  async setup(): Promise<void> {
    let event = this.apiQueue.eventDetails.get(this.slug);
    const now = Date.now();
    const retryMs = Math.max(500, Number(process.env.EVENT_DETAILS_RETRY_MS ?? 1500));

    // V13: next-slot prewarm must not hammer Gamma API every 100ms while
    // the future market is not published yet.
    if (!event) {
      if (now - this._lastSetupAttemptMs < retryMs) return;
      this._lastSetupAttemptMs = now;
      await this.apiQueue.queueEventDetails(this.slug);
      event = this.apiQueue.eventDetails.get(this.slug);
    }

    if (!event) {
      if (now - this._lastSetupLogMs > 5000) {
        this._lastSetupLogMs = now;
        this._log(`[${this.slug}] V13: waiting for event metadata / market not published yet`, "dim");
      }
      return;
    }
  ...
  private async _handleInit(): Promise<void> {
    ...
    const fastMode = ["1", "true", "yes", "on"].includes((process.env.ULTRA_FAST_MODE ?? "").toLowerCase());
    const prewarmMs = Math.max(0, Number(process.env.MARKET_PRICE_PREWARM_MS ?? (fastMode ? 1800 : 0)));
    const delayMs = Math.max(0, slot.startTime - Date.now() - prewarmMs);
    ...
    const orderbookReady = await this._orderBook.waitForReady();
    if (!orderbookReady) {
      this._log(
        `[${this.slug}] V13 WARN: orderbook not ready after ${process.env.ORDERBOOK_READY_TIMEOUT_MS ?? 3500}ms; starting strategy anyway so dashboard keeps live diagnostics.`,
        "yellow",
      );
    }
  ...
  private async _processPendingOrders(): Promise<void> {
    ...
    // V14: in ultra-fast mode reduce index grace so stale/unindexed GTC orders do not block a 5m cycle.
    const fastMode = ["1", "true", "yes", "on"].includes((process.env.ULTRA_FAST_MODE ?? "").toLowerCase());
    const CLOB_INDEX_GRACE_MS = Math.max(100, Number(process.env.CLOB_INDEX_GRACE_MS ?? (fastMode ? 900 : 5000)));
  ...
  private _postOrders(requests: OrderRequest[]): void {
    ...
    const fastMode = ["1", "true", "yes", "on"].includes((process.env.ULTRA_FAST_MODE ?? "").toLowerCase());
    const retryMs = Math.max(25, Number(process.env.ORDER_RETRY_MS ?? (fastMode ? 80 : 500)));
    const buyRetries = Math.max(1, Number(process.env.ORDER_BUY_MAX_RETRIES ?? (fastMode ? 8 : 30)));
    if (buys.length > 0) this._placeWithRetry(buys, retryMs, buyRetries);
    if (sells.length > 0) this._placeWithRetry(sells, retryMs, Infinity);
  }
