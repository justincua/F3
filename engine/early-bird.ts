...
import { clearStopRequest, readBotControl } from "./bot-control.ts";

const FAST_MODE = ["1", "true", "yes", "on"].includes((process.env.ULTRA_FAST_MODE ?? "").toLowerCase());
const SAVE_INTERVAL_MS = Math.max(1000, Number(process.env.STATE_SAVE_INTERVAL_MS ?? (FAST_MODE ? 15000 : 5000)));
...
function readWatchOffsets(baseOffset: number): number[] {
  const raw = process.env.SLOT_WATCH_OFFSETS;
  if (raw && raw.trim()) {
    const parsed = raw
      .split(",")
      .map((v) => Number(v.trim()))
  ...
  // V13: Railway sometimes lags exactly around the 5m boundary.
  // Keep the current slot and the next slot warm by default.
  if (boolEnv("PREWARM_NEXT_SLOT", true)) return [baseOffset, baseOffset + 1];
  return [baseOffset];
}

function readMaxActiveLifecycles(): number {
  const raw = Number(process.env.MAX_ACTIVE_LIFECYCLES ?? 3);
...
export class EarlyBird {
  ...
  private readonly _watchOffsets: number[];
  private readonly _maxActiveLifecycles: number;
  ...
  constructor(
    ...
    this._watchOffsets = readWatchOffsets(slotOffset);
    this._maxActiveLifecycles = readMaxActiveLifecycles();
  ...
  async start(): Promise<void> {
    ...
    this._tracker = new WalletTracker(
      initialBalance,
      (msg) => log.write(msg, "dim"),
      (snapshot) => dashboardState.updateWallet(snapshot),
    );

    dashboardState.startSession({
      strategy: this._strategyName,
      mode: this._prod ? "PROD" : "SIM",
      asset: Env.getAssetConfig().apiSymbol,
    ...
    log.write(
      `[startup] V13 slot watcher offsets=${this._watchOffsets.join(",")} maxActive=${this._maxActiveLifecycles}`,
      "cyan",
    );
    dashboardState.setStatus("Đang chạy realtime");
    ...
    const onSignal = (sig: string) => {
      ...
      dashboardState.setStatus(`Đang tắt bot: ${sig}`);
      log.flush();
      this._saveState();
    ...
    const engineTickMs = Math.max(15, Number(process.env.ENGINE_TICK_MS ?? (FAST_MODE ? 40 : 100)));
    log.write(`[startup] V14 ultra speed: tick=${engineTickMs}ms save=${SAVE_INTERVAL_MS}ms fast=${FAST_MODE ? "ON" : "OFF"}`, FAST_MODE ? "cyan" : "dim");
    setInterval(() => this._tick(), engineTickMs);
  }

  private _tick(): void {
    const control = readBotControl();
    dashboardState.updateControl({
    ...
    if (control.stopRequested && !this._shuttingDown) {
      dashboardState.setStatus("Nhận lệnh STOP từ dashboard");
      clearStopRequest();
      this._startShutdown("Stop requested from dashboard.");
    }

    if (control.paused && !this._shuttingDown) {
      dashboardState.setStatus("Đang PAUSE - không tạo market mới");
      for (const lifecycle of this._lifecycles.values()) {
        if (lifecycle.state === "INIT" || lifecycle.state === "RUNNING") {
          lifecycle.shutdown();
        }
      }
    }

    // V13: slot watcher. Railway can lag/restart exactly on a 5m boundary.
    // Instead of watching only the current slot, keep current + next slot warm.
    const roundsExhausted =
      this._rounds !== null && this._roundsCreated >= this._rounds;
    if (!control.paused && !this._shuttingDown && !roundsExhausted) {
      const createdNow: string[] = [];
      for (const offset of this._watchOffsets) {
        if (this._lifecycles.size >= this._maxActiveLifecycles) break;
        const slug = getSlug(offset);
        if (this._lifecycles.has(slug) || this._completedSlugs.has(slug)) continue;
        ...
        this._roundsCreated++;
        createdNow.push(`${slug}(+${offset})`);
        if (this._rounds !== null && this._roundsCreated >= this._rounds) break;
      }
      if (createdNow.length > 0) {
        log.write(`[slot-watch] Created lifecycle(s): ${createdNow.join(", ")}`, "dim");
      }
    }
    ...
    const activeLivePnl = [...this._lifecycles.values()].reduce(
      (sum, lifecycle) => sum + lifecycle.livePnl,
      0,
    );
    const liveSessionPnl = parseFloat((this._sessionPnl + activeLivePnl).toFixed(4));
    const liveSessionLoss = parseFloat((this._sessionLoss + Math.min(0, activeLivePnl)).toFixed(4));

    dashboardState.updateCore({
      sessionPnl: liveSessionPnl,
      sessionLoss: liveSessionLoss,
      wins: this._completedMarkets.filter((m) => m.pnl > 0).length,
      losses: this._completedMarkets.filter((m) => m.pnl < 0).length,
      completedMarkets: this._completedMarkets.length,
      activeMarkets: [...this._lifecycles.values()].map((lifecycle) =>
        lifecycle.getDashboardSnapshot(),
      ),
    });
    ...
    if (this._shuttingDown && this._lifecycles.size === 0) {
      ...
      dashboardState.setStatus("Đã dừng");
      dashboardState.updateControl({ process: "stopped" });
      this._saveState();
  ...
  private _startShutdown(reason: string): void {
    ...
    dashboardState.setStatus(`Đang dừng: ${reason}`);
