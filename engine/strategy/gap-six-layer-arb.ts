...
type BotMode = "IDLE" | "PURE_ARB" | "GAP_FIRST_LEG" | "WAIT_HEDGE" | "LOCKED" | "RESCUE" | "UNLOCK";
...
type HybridConfig = {
  ...
  maxRescueSum: number;
  realtimeEverything: boolean;
  unlockEnabled: boolean;
  unlockIfSumAbove: number;
  unlockAfterMs: number;
  unlockBeforeCloseSecs: number;
  keepOneSideMaxMs: number;
  cooldownMs: number;
...
function readConfig(): HybridConfig {
  const ultraFast = boolEnv("ULTRA_FAST_MODE", false);
  ...
  return {
    ...
    scanMs: Math.max(10, Math.floor(numEnv("GAP_SCAN_MS", numEnv("ARB_SCAN_MS", ultraFast ? 25 : 100)))),
    fillTimeoutMs: Math.max(150, Math.floor(numEnv("GAP_FIRST_FILL_TIMEOUT_MS", numEnv("ARB_FILL_TIMEOUT_MS", ultraFast ? 700 : 1500)))),
    hedgeFillTimeoutMs: Math.max(150, Math.floor(numEnv("GAP_HEDGE_FILL_TIMEOUT_MS", numEnv("ARB_FILL_TIMEOUT_MS", ultraFast ? 700 : 1500)))),
    ...
    realtimeEverything: boolEnv("REALTIME_EVERYTHING", boolEnv("FULL_REALTIME", ultraFast)),
    unlockEnabled: boolEnv("UNLOCK_ENABLED", true),
    unlockIfSumAbove: numEnv("UNLOCK_IF_SUM_ABOVE", numEnv("HOLD_ONE_SIDE_SUM_LIMIT", 1.03)),
    unlockAfterMs: Math.max(1000, Math.floor(numEnv("UNLOCK_AFTER_MS", 45000))),
    unlockBeforeCloseSecs: Math.max(3, Math.floor(numEnv("UNLOCK_BEFORE_CLOSE_SECS", 120))),
    keepOneSideMaxMs: Math.max(1000, Math.floor(numEnv("KEEP_ONE_SIDE_MAX_MS", 45000))),
    cooldownMs: Math.max(0, Math.floor(numEnv("GAP_COOLDOWN_MS", ultraFast ? 0 : 1000))),
...
function pushStatus(params: {
  ...
  ctx.setDiagnostics({
    ...
    arb: {
      ...
      realtimeEverything: cfg.realtimeEverything,
      unlockEnabled: cfg.unlockEnabled,
      unlockIfSumAbove: cfg.unlockIfSumAbove,
    ...
    if (firstLeg && firstLeg.status === "filled" && firstLeg.filledShares > firstLeg.hedgeShares) {
      ...
      ctx.setDiagnostics({
        ...
        arb: {
          ...
          riskNote: `Đang giữ 1 phe: chưa phải arbitrage. V15 realtime: nếu SUM <= ${lockSumLimit(cfg).toFixed(4)} thì hedge ngay; nếu SUM > ${cfg.unlockIfSumAbove.toFixed(2)} quá lâu sẽ unlock để không khóa lỗ sâu.`,
          ...
          unlockEnabled: cfg.unlockEnabled,
          unlockIfSumAbove: cfg.unlockIfSumAbove,
          unlockAfterMs: cfg.unlockAfterMs,
        },
      });

      // V15: Realtime safety for SUM > 1.03.
      // Nếu hedge hiện tại sẽ khóa lỗ sâu (SUM lớn hơn ngưỡng), KHÔNG mua phe còn lại bừa.
      // Chỉ giữ một bên trong thời gian ngắn; quá thời gian hoặc gần hết giờ thì bán/unlock first-leg.
      const sumAboveUnlock = currentSum !== null && currentSum > cfg.unlockIfSumAbove;
      const shouldUnlockBadSum = cfg.unlockEnabled
        && needShares > 0
        && sumAboveUnlock
        && (age >= cfg.unlockAfterMs || age >= cfg.keepOneSideMaxMs || remaining <= cfg.unlockBeforeCloseSecs);

      if (shouldUnlockBadSum) {
        const reason = `SUM ${currentSum?.toFixed(4) ?? "--"} > ${cfg.unlockIfSumAbove.toFixed(2)} quá lâu (${Math.round(age / 1000)}s), unlock first-leg`;
        firstLeg.status = "rescue";
        firstLeg.reason = reason;
        ctx.log(`[${ctx.slug}] gap-arb: UNLOCK ${prettySide(firstLeg.side)} | ${reason}`, "yellow");
        ctx.setDiagnostics({
          pipeline: pipeline({
            scan: { state: "pass", detail: "Realtime theo dõi first-leg" },
            detect: { state: "warn", detail: `SUM hiện tại ${currentSum?.toFixed(4) ?? "--"} > ${cfg.unlockIfSumAbove.toFixed(2)}` },
            validate: { state: "warn", detail: "Không hedge vì sẽ khóa lỗ sâu" },
            size: { state: "pass", detail: `${needShares.toFixed(2)} shares cần unlock` },
            fill: { state: "warn", detail: "Bán lại first-leg" },
            hedge: { state: "warn", detail: "Unlock thay vì hedge > 1.03" },
          }),
          arb: {
            mode: "UNLOCK",
            lockSum: currentSum,
            lockedEdgeCents: currentEdge,
            rescueReason: reason,
            lastAction: `Unlock ${prettySide(firstLeg.side)} vì SUM > ${cfg.unlockIfSumAbove.toFixed(2)}`,
            waitReason: reason,
          } as any,
        });
