...
export type ArbSnapshot = {
  ...
  mode?: "PURE_ARB" | "GAP_FIRST_LEG" | "WAIT_HEDGE" | "LOCKED" | "RESCUE" | "UNLOCK" | "IDLE" | null;
  ...
  // V15: realtime + unlock one-side risk control.
  realtimeEverything?: boolean | null;
  unlockEnabled?: boolean | null;
  unlockIfSumAbove?: number | null;
...
class DashboardStateWriter {
  ...
  flush(force = false) {
    const now = Date.now();
    const minFlushMs = Number(process.env.REALTIME_STATE_FLUSH_MS ?? (process.env.REALTIME_EVERYTHING === "true" ? 80 : 250));
    if (!force && now - this._lastFlushMs < minFlushMs) return;
